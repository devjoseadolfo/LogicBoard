...
public class Xor: Gate, Expandable {
    public var parentCircuit: Circuit?
    
    public var deviceName: String = "XOR GATE"
    public var imageName: String = "XOR"
    ...
    public init() {
        ...
        imageNode.position = .init(x: 4, y: 0)
    ...
    public func run() {
        outputs[0] = inputs.filter({$0}).count % 2 == 1
    }
}
